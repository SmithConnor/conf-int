...
$$
CI = ( `r round(mean2_dice - 1.96*sd(sample2_dice)/sqrt(n2), 2)` , `r round(mean2_dice + 1.96*sd(sample2_dice)/sqrt(n2), 2)` )
$$
...
df <- data.frame(xseq = seq(10.15-4,10.15+4,.01), densities = dnorm(seq(10.15-4,10.15+4,.01), 10.15, 4.15/sqrt(30)))
...
::: {.column width="50%"}
\begin{align}
P(\mu < L(X)) & = P(\mu < `r round(mean2_dice - 1.96*sd(sample2_dice)/sqrt(n2), 2)`)\\
& = 0.025
\end{align}

```{r}
#| echo: true
pnorm(7.18, 8.67, 4.15/sqrt(30))
```
:::

::: {.column width="50%"}
\begin{align}
P(\mu > U(X)) & = P(\mu > `r round(mean2_dice + 1.96*sd(sample2_dice)/sqrt(n2), 2)`)\\
& = 0.025
\end{align}

```{r}
#| echo: true
1 - pnorm(10.15, 8.67, 4.15/sqrt(30))
```
:::
...
\begin{align}
P(L(X) < \mu < U(X)) & = P(7.18 < \mu < 10.15)\\
& = 1 - P(\mu < 7.18) - P(\mu > 10.15)\\
& = 0.95
\end{align}

```{r}
#| echo: true
1 - pnorm(7.18, 8.67, 4.15/sqrt(30)) - (1 - pnorm(10.15, 8.67, 4.15/sqrt(30)))
```
